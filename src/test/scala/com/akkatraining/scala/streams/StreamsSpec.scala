package com.akkatraining.scala.streams
...
import akka.stream.{ActorMaterializer, Attributes, IOResult, OverflowStrategy}
...
class StreamsSpec extends FunSuite with Matchers {
  ...
  test("""Case 21: Perform a stream with a recoverWithRetries which will run the test a specified
    ...
    Source(5 to 0 by -1)
      .async
      .map{x => Some(100 / x)}
      .recoverWithRetries(3, { case t: Throwable => Source(3 to 0 by -1).map(y => Some(100/y))})
      .runForeach(println)
    Thread.sleep(5000)
  }
